#!/usr/bin/env bash
# Falha o CI se pubspec, Version.xcconfig e MH_IOS_BUILD.txt divergirem.
set -euo pipefail
...
test -f ios/MH_IOS_BUILD.txt || { echo "ERRO: ios/MH_IOS_BUILD.txt ausente"; exit 1; }
TXT_NUM=$(tr -d ' \n\r' < ios/MH_IOS_BUILD.txt)

echo "pubspec: ${PUBSPEC_NAME}+${PUBSPEC_NUM}"
echo "Version.xcconfig: ${XC_NAME}/${XC_NUM}"
echo "MH_IOS_BUILD.txt: ${TXT_NUM}"
...
if [ "$TXT_NUM" != "$PUBSPEC_NUM" ]; then
  echo "ERRO: MH_IOS_BUILD.txt (${TXT_NUM}) != pubspec (+${PUBSPEC_NUM})"
  exit 1
fi
if [ -n "${MH_BUILD_NUMBER:-}" ] && [ "$MH_BUILD_NUMBER" != "$PUBSPEC_NUM" ]; then
  echo "ERRO: MH_BUILD_NUMBER (${MH_BUILD_NUMBER}) != pubspec (+${PUBSPEC_NUM})"
  exit 1
fi
...
if [ -f "$ASC_FILE" ]; then
  ...
  if [ -n "$ASC_LAST" ] && [ "$PUBSPEC_NUM" -le "$ASC_LAST" ]; then
    ...
    echo "SOLUCAO: incremente para +$((ASC_LAST + 1)) em pubspec, Version.xcconfig, MH_IOS_BUILD.txt e codemagic.yaml"
    exit 1
  fi
