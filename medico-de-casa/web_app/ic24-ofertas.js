/* Idoso Care 24H — ofertas, painel família/contratante, cruzamento médico */
...
function ic24TermosRecebimentoVisita(rate) {
  return {
    paymentSchedule: 'diaria',
    paymentWeekDay: 'seg',
    jobDurationDays: 1,
    configured: true,
    scheduleLabel: 'Visita única — pagamento ao fim da consulta',
    diariasCount: 1,
    perCycleAmount: Number(rate) || 0,
    totalContractAmount: Number(rate) || 0,
    paymentCyclesTotal: 1,
  };
}
...
async function ic24ResolverFamilyIdDoCuidador(caregiverId) {
  ...
  if (!cgId) return null;
  const cgSnap = await ic24Db.collection('doctors').doc(cgId).get();
...
const ic24ResolverFamilyIdDoMédico = ic24ResolverFamilyIdDoCuidador;

async function ic24VincularFamiliaAtiva(caregiverId, familyId) {
  if (!caregiverId || !familyId) return;
  ic24InitFirebase();
  await ic24Db.collection('doctors').doc(caregiverId).set(
...
function ic24TextoNotificacaoFamilia(caregiverName, r) {
  const nome = caregiverName || 'O médico';
...
async function ic24NotificarFamiliaProposta(familyId, offerId, responseId, message, caregiverId, caregiverName) {
  ...
  await ic24Db.collection('family_notifications').add({
    ...
    caregiverName: caregiverName || 'O médico',
    message,
    read: false,
    status: 'pending',
    createdAt: firebase.firestore.FieldValue.serverTimestamp(),
...
async function ic24ObterPreferenciasRecebimento(caregiverId) {
  ...
  if (!id) throw new Error('Médico não identificado');
  const cgSnap = await ic24Db.collection('doctors').doc(id).get();
...
async function ic24AceitarOfertaComTermos(offerId, payload) {
  ...
  if (ic24IsVisitOffer(offer)) {
    const rate = payload?.proposedDailyRate || offer.dailyRate || offer.consultationRate;
    return ic24FinalizarFormaRecebimento(r1.id, ic24TermosRecebimentoVisita(rate));
  }
  const terms = await ic24ObterPreferenciasRecebimento();
  return ic24FinalizarFormaRecebimento(r1.id, terms);
}

/** Família envia proposta direta a um médico. */
async function ic24FamiliaProporCuidador(caregiverId, { dailyRate, message, durationDays, elderlyType, careNeeds }) {
  ic24InitFirebase();
  const familyId = ic24Auth.currentUser?.uid;
  if (!familyId) throw new Error('Faça login como família');
  if (!caregiverId) throw new Error('Médico não informado');
  ...
  const data = {
    ...
    title: 'Proposta para ' + (cgSnap.data().fullName || 'médico'),
    dailyRate,
    careNeeds: careNeeds || '',
...
async function ic24CarregarPerfilCuidadorPublico(caregiverId) {
  ic24InitFirebase();
  const snap = await ic24Db.collection('doctors').doc(caregiverId).get();
  if (!snap.exists) throw new Error('Médico não encontrado');
  const d = snap.data();
  return {
    id: caregiverId,
    fullName: d.fullName || 'Médico',
    city: d.city || '',
    state: d.state || '',
    bio: d.bio || '',
...
async function ic24Passo1Oferta(offerId, payload) {
  ...
  if (!caregiverId) throw new Error('Faça login como médico');
  const cgSnap = await ic24Db.collection('doctors').doc(caregiverId).get();
...
/** Passo 2: médico escolhe como quer receber e envia ao contratante. */
async function ic24FinalizarFormaRecebimento(responseId, terms) {
  ...
  const visitTerms = ic24IsVisitOffer(offer);
  const durationDays = visitTerms ? 1 : terms.jobDurationDays;
  const calc =
    typeof ic24CalcularPropostaPagamento === 'function' && !visitTerms
      ? ic24CalcularPropostaPagamento(rate, terms.paymentSchedule, terms.paymentWeekDay, durationDays)
      : visitTerms
        ? ic24TermosRecebimentoVisita(rate)
        : terms;
  const patch = {
...
async function ic24FamiliaAceitarContraProposta(responseId, accept, notificationId) {
  ...
  if (accept) {
    ...
    if (typeof ic24AcumularTaxaPlataforma === 'function') {
      const diarias = ic24VisitCountForFee(offer, r);
...
async function ic24CriarAvaliacao({ caregiverId, nota, texto, offerId }) {
  ...
  const cgRef = ic24Db.collection('doctors').doc(caregiverId);
