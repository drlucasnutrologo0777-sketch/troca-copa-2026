...
/** Taxa por visita — Apple IAP consumível ic24_taxa_manutencao (1 unidade = 1 visita) */
const IC24_FEE_PER_DIARIA_USD = 4.99;
const IC24_FEE_PER_DIARIA_BRL = 25;
...
async function ic24AcumularTaxaPlataforma(caregiverId, diariasCount, offerId) {
  ...
  ic24InitFirebase();
  const ref = ic24Db.collection('doctors').doc(caregiverId);
...
async function ic24PrefillCobrancaDoPlantao() {
  ...
  if (!caregiverId) return { canGenerate: false, reason: 'Faça login como médico' };
...
async function ic24GerarCobrancaCliente({ valor, descricao, metodo, linkCartao, familyId, pixKey, pixTitular, offerId, paymentCycle }) {
  ...
  const data = {
    ...
    amount,
    description: descricao || 'Serviço de médico',
    method: metodo || 'pix',
    status: 'pending',
    txid,
    pixTitular: titular,
  ...
  if (chavePix && chavePix !== cg.pixKey) {
    await ic24Db.collection('doctors').doc(caregiverId).set({ pixKey: chavePix, pixTitular: titular }, { merge: true });
  }
...
async function ic24RegistrarPagamentoTaxa(uid, iap, offerId) {
  const cgSnap = await ic24Db.collection('doctors').doc(uid).get();
  ...
  const pay = {
    ...
    note: fullyPaid ? 'Apple StoreKit — taxa quitada' : 'Apple StoreKit — 1 visita paga',
    createdAt: firebase.firestore.FieldValue.serverTimestamp(),
  };
  await ref.set(pay);
  ...
  await ic24Db.collection('doctors').doc(uid).set(cgPatch, { merge: true });
...
async function ic24PagarTaxaPlataforma(metodo) {
  ...
  const snap = await ic24Db.collection('doctors').doc(uid).get();
  const cg = snap.data() || {};
  ...
  if (units > 1 && typeof confirm === 'function') {
    const ok = confirm(
      'Taxa pendente: ' +
        units +
        ' visita(s) × US$ ' +
        IC24_FEE_FIXED_USD.toFixed(2) +
        ' = ' +
        ic24FmtTaxaUsd(pending) +
        '.\n\nSerão ' +
        units +
        ' compras na App Store (US$ ' +
        IC24_FEE_FIXED_USD.toFixed(2) +
        ' cada). Continuar?',
    );
  ...
  for (let i = 0; i < units; i++) {
    ...
    const snap2 = await ic24Db.collection('doctors').doc(uid).get();
...
function ic24BaixarCurriculoCadastro(d, cls, docsMap) {
  ...
  const html =
    ...
    '<h1>Currículo Lates — Medico de Casa</h1>' +
    ...
    ' — Medico de Casa</small></p></body></html>';
  const blob = new Blob([html], { type: 'text/html;charset=utf-8' });
  const a = document.createElement('a');
  a.href = URL.createObjectURL(blob);
