@echo off
echo Build 47: 2.0.0+47 — agenda/plantao com valor na publicacao + taxa US$ 1,99 IAP.
echo Faca push do main antes: git push origin main
echo.
start "" "https://codemagic.io/apps"
start "" "https://github.com/drlucasnutrologo0777-sketch/troca-copa-2026"
start "" "https://appstoreconnect.apple.com/apps/6784357547/testflight/ios"
echo.
echo Codemagic: troca-copa-2026 ^> Start new build ^> **Idoso Care 24H — iOS TestFlight** ^> branch main
pause
