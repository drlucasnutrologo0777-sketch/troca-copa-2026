#!/usr/bin/env bash
# Falha o CI se pubspec, Version.xcconfig e IC24_IOS_BUILD.txt divergirem.
set -euo pipefail
...
test -f ios/IC24_IOS_BUILD.txt || { echo "ERRO: ios/IC24_IOS_BUILD.txt ausente"; exit 1; }
TXT_NUM=$(tr -d ' \n\r' < ios/IC24_IOS_BUILD.txt)

echo "pubspec: ${PUBSPEC_NAME}+${PUBSPEC_NUM}"
echo "Version.xcconfig: ${XC_NAME}/${XC_NUM}"
echo "IC24_IOS_BUILD.txt: ${TXT_NUM}"
...
if [ "$TXT_NUM" != "$PUBSPEC_NUM" ]; then
  echo "ERRO: IC24_IOS_BUILD.txt (${TXT_NUM}) != pubspec (+${PUBSPEC_NUM})"
  exit 1
fi
if [ -n "${IC24_BUILD_NUMBER:-}" ] && [ "$IC24_BUILD_NUMBER" != "$PUBSPEC_NUM" ]; then
  echo "ERRO: IC24_BUILD_NUMBER (${IC24_BUILD_NUMBER}) != pubspec (+${PUBSPEC_NUM})"
  exit 1
fi
if [ "$PUBSPEC_NUM" -le 66 ]; then
  echo "ERRO: build ${PUBSPEC_NUM} <= 66 — Apple rejeita DUPLICATE (66 ja no TestFlight)"
  exit 1
fi
