/* Babá ON — cobrança PIX/cartão babá↔mãe/pai (diária) + taxa plataforma EXCLUSIVA Apple IAP */

/** Taxa por diária — Apple IAP consumível bo_taxa_manutencao (1 unidade = 1 diária) */
...
async function ic24ObterOfertaAtivaBaba(caregiverId) {
  ic24InitFirebase();
...
async function ic24PrefillCobrancaDoPlantao() {
  ...
  if (!caregiverId) return { canGenerate: false, reason: 'Faça login como babá' };
  const offer = await ic24ObterOfertaAtivaBaba(caregiverId);
...
async function ic24NotificarBabaPagamento(caregiverId, invoice) {
...
async function ic24ConfirmarPagamentoFamilia(invoiceId) {
  ...
  if (inv.caregiverId) await ic24NotificarBabaPagamento(inv.caregiverId, paid);
  if (inv.offerId) {
...
async function ic24ListarNotificacoesBaba(limit) {
  ic24InitFirebase();
...
async function ic24SalvarDadosCobranca({ pixKey, pixTitular, linkCartao }) {
  ic24InitFirebase();
  const caregiverId = ic24Auth.currentUser?.uid;
  if (!caregiverId) throw new Error('Faça login como babá');
  const chavePix = (pixKey || '').trim();
  const titular = (pixTitular || '').trim();
  const card = (linkCartao || '').trim();
  const patch = {};
  if (chavePix) patch.pixKey = chavePix;
  if (titular) patch.pixTitular = titular;
  patch.linkCartao = card || null;
  await ic24Db.collection('caregivers').doc(caregiverId).set(patch, { merge: true });
  return patch;
}
...
async function ic24GerarCobrancaCliente({ valor, descricao, metodo, linkCartao, familyId, pixKey, pixTitular, offerId, paymentCycle }) {
  ...
  const cardLink = (linkCartao || cg.linkCartao || '').trim() || null;
  const data = {
    id: ref.id,
    caregiverId,
  ...
  const cgPatch = {};
  if (chavePix) {
    cgPatch.pixKey = chavePix;
    cgPatch.pixTitular = titular;
  }
  if (cardLink) cgPatch.linkCartao = cardLink;
  if (Object.keys(cgPatch).length) {
    await ic24Db.collection('caregivers').doc(caregiverId).set(cgPatch, { merge: true });
  }
...
async function ic24ListarCobrancasBaba() {
  ic24InitFirebase();
  const uid = ic24Auth.currentUser?.uid;
  if (!uid) return [];
  const snap = await ic24QuerySnap(
...
async function ic24ResumoFinanceiroBabá() {
  const invs = await ic24ListarCobrancasBaba();
...
async function ic24ListarTrabalhosBabá(caregiverId) {
  ic24InitFirebase();
  const cgId = caregiverId || ic24Auth.currentUser?.uid;
  if (!cgId) return [];
  const snap = await ic24Db
...
async function ic24RegistrarPagamentoTaxa(uid, iap, offerId) {
  ...
  const pay = {
    ...
    status: 'confirmed',
    txid,
    appleProductId: (iap && iap.productId) || 'bo_taxa_manutencao',
...
async function ic24ListarTaxasPlataformaBabá() {
  ic24InitFirebase();
  const uid = ic24Auth.currentUser?.uid;
  if (!uid) return [];
  const snap = await ic24QuerySnap(
