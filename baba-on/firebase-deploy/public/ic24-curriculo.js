/* Babá ON — documentos (Storage), classificação e currículo público */
...
async function ic24MontarCurriculoSnapshot(uid, opts = {}) {
  ...
  if (!cgSnap.exists) throw new Error('Babá não encontrada');

  const cg = cgSnap.data() || {};

  if (!cg.fullName && !String(cg.cpf || '').replace(/\D/g, '')) {

    throw new Error('Currículo ainda não disponível — babá precisa completar o cadastro');
...
/** URL absoluta para curriculo.html (file:// no iPhone quebra location.origin). */

function ic24ResolveCurriculoUrl(relativeLink) {

  const link = String(relativeLink || '').trim();
...
async function ic24SolicitarCurriculo(caregiverId) {
  ...
  if (!familyId) throw new Error('Faça login como mãe/pai');

  if (!caregiverId) throw new Error('Selecione uma babá');
...
async function ic24CarregarCurriculoPorToken(token) {
  ...
  delete curriculum.street;

  delete curriculum.number;

  delete curriculum.neighborhood;

  delete curriculum.cep;
...
async function ic24ListarBabasAprovados() {
