...
import '../constants/bo_iap_config.dart';

/// StoreKit — compra consumível da taxa de manutenção (ponte WebView → nativo).
class BoIapService {
  BoIapService._();
  static final instance = BoIapService._();
  ...
  String get precoExibicao => _product?.price ?? BoIapConfig.precoFallback;
  ...
  Future<void> _carregarProduto() async {
    final response = await _iap.queryProductDetails({BoIapConfig.platformFeeProductId});
  ...
  Future<Map<String, dynamic>> comprarTaxaManutencao({String? caregiverId}) async {
    ...
    if (_product == null) {
      throw StateError(
        'Produto "${BoIapConfig.platformFeeProductId}" não encontrado. '
  ...
  void _onPurchaseUpdate(List<PurchaseDetails> purchases) {
    for (final purchase in purchases) {
      if (purchase.productID != BoIapConfig.platformFeeProductId) continue;
