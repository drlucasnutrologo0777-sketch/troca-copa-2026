...
echo "=== Verificar codigo fonte Babá ON ==="
...
grep -q 'bo_taxa_manutencao' web_app/ic24-cobranca.js || { echo "ERRO: IAP bo_taxa_manutencao ausente"; exit 1; }
grep -q 'Protótipo Web' web_app/index.html && { echo "ERRO: badge Protótipo Web (Apple rejeita)"; exit 1; } || true
grep -q 'Conselho da Pessoa Idosa' web_app/index.html && { echo "ERRO: texto idoso no app babá"; exit 1; } || true
BUILD_STAMP=$(grep '^version:' pubspec.yaml | sed 's/version: /Babá ON iOS build /')
grep -q "$BUILD_STAMP" web_app/index.html || {
  echo "ERRO: build stamp incorreto — esperado comentario: $BUILD_STAMP"
  exit 1
}
echo "OK: fonte aponta para WebAppScreen + web_app/ Babá ON ($BUILD_STAMP)"
